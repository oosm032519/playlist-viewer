            ...
            {options.map((option) => (
                <div className="mx-2" key={option.id}>
                    ...
                    <label htmlFor={option.id}
                           className={`inline-block p-2 px-5 m-2 border-2 border-green-500 rounded-full ${selectedOption === option.value ? 'bg-green-500 text-white' : 'bg-white text-green-500'} transition-all duration-300 ease-in-out cursor-pointer w-11/10 hover:bg-green-500 hover:text-white hover:shadow-md hover:border-transparent`}>
                        {option.label}
                    </label>
                </div>
            ))}
